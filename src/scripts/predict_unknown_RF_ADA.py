...
def check_predictions(_model_RF, _model_ADA, trainPortion):
    ...
    for idx, tx in enumerate(contents):
        ...
        predictions[tx["hash"]] = (
            predict(tx["hash"], scaler, _model_RF, rawTx=tx),
            predict(tx["hash"], scaler, _model_ADA, rawTx=tx),
        )

    with open(
        f"predictions_comparison_{trainPortion}_{100-trainPortion}.csv", "w", newline=""
    ) as csvfile:
