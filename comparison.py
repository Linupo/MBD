...
# General info
print(f"[Mine] Predicted legal transactions {len(predictions_mine.loc[predictions_mine['predictionXG'] == True])}")
print(f"[Mine] Predicted illegal transactions {len(predictions_mine.loc[predictions_mine['predictionXG'] == False])}")
...
for idx in range(len(predictions_mine)):
    predictions_mine_map[predictions_mine['txHash'][idx]] = 0 if predictions_mine['predictionXG'][idx] else 1
